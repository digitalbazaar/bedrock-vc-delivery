...
- **Multi-protocol support**: VC-API, OID4VCI (draft 13 and v1.0), OID4VP
  (draft 18 and v1.0), and Invite Request.
...
| `jwtDidProofRequest` | Request a DID-bound JWT proof (cannot combine with `openId`) |
...
"openId": {
  "clientProfiles": {
    "myProfile": {
      ...
      "presentation_definition": { /* PD object */ },
...
- `jwt_vc_json-ld` (JSON-LD-based VC-JWT, issued as an envelope)

Both OID4VCI Draft 13 and OID4VCI v1.0 credential request formats are
supported.
...
- OID4VP Draft 18 (compatible with older wallets)
