      ...
      if(exchange.step) {
        ...
        if(step.jwtDidProofRequest) {
          ...
          if(!(req?.body?.proof?.jwt)) {
            return await _requestDidProof({res, exchanger});
          }
          // verify the DID proof
          const {body: {proof: {jwt}}} = req;
          const result = await verifyDidProofJwt({exchanger, exchange, jwt});
          // FIXME: handle result
        }
      }
...
async function _requestDidProof({res, exchanger, exchange}) {
  ...
  // FIXME: OIDC4VCI exchanges themselves are not replayable and single-step,
  // so can the challenge just come from the exchange ID itself? once consumed,
  // the challenge and exchange cannot be used again; must allow for there to
  // be an expected DID -- for exchanges that
