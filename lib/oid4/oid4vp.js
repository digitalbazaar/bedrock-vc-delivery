...
export async function processAuthorizationResponse({req, clientProfileId}) {
  ...
  const exchangeProcessor = new ExchangeProcessor({
    ...
    async verify({
      ...
      if(parseResponseResult?.mdocOptions) {
        // get expected `handover` for mdoc verification
        const {expectedHandover: handover} = parseResponseResult.mdocOptions;

        // handle deprecated `mdl` option; `mdoc` is preferred
        const mdocKey = envelope.mediaType === 'application/mdoc-vp-token' ?
          'mdoc' : 'mdl';

        verifyPresentationOptions[mdocKey] = {
          ...verifyPresentationOptions[mdocKey],
          // send encoded mdoc `sessionTranscript`
