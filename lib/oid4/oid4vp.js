...
export async function processAuthorizationResponse({req}) {
  ...
  try {
    ...
    const stepResult = {
      ...
      // include unenveloped credentials
      unenvelopedCredentials: verifyResult.credentialResults?.map(
        r => r.format ? r.credential : null).filter(r => !!r)
    };
    if(envelope) {
      // normalize VP from inside envelope to `verifiablePresentation`
      stepResult.envelopedPresentation = presentation;
      stepResult.verifiablePresentation = verifyResult
