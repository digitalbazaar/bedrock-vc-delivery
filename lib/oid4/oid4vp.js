...
export async function getAuthorizationRequest({req, clientProfileId}) {
  ...
  while(true) {
    ...
    const clientProfile = _getClientProfile({step, clientProfileId});
...
export async function getOID4VPProtocols({workflow, exchange}) {
  ...
  if(!step.openId.clientProfiles) {
    const clientBaseUrl = _getClientBaseUrl({workflow, exchange});
  ...
  for(const [clientProfileId, clientProfile] of clientProfiles) {
    ...
    const clientBaseUrl = _getClientBaseUrl({
...
function _createAuthorizationRequest({
  workflow, exchange,
  clientProfile, clientProfileId,
  verifiablePresentationRequest
}) {
  ...
  if(_usesX509SanDns({authorizationRequest}) &&
    authorizationRequest.response_mode === 'direct_post') {
...
function _createClientMetaData({authorizationRequest} = {}) {
  ...
  if(authorizationRequest.response_mode === 'direct_post.jwt') {
    ...
    */
    ...
    if(_requestsMdoc({authorizationRequest})) {
      vp_formats.mso_mdoc = {
        alg: ['EdDSA', 'ES256']
      };
    }
...
function _getClientBaseUrl({workflow, exchange, clientProfileId}) {
...
function _getClientProfile({step, clientProfileId}) {
...
// FIXME: move to `oid4-client` lib?
function _requestsMdoc({authorizationRequest}) {
  ...
  */
  return authorizationRequest.presentation_definition?.input_descriptions.some(
    e => e?.format?.mso_mdoc);
}
...
function _usesX509SanDns({authorizationRequest}) {
  return authorizationRequest.client_id_scheme === 'x509_san_dns' ||
    authorizationRequest.client_id?.startsWith('x509_san_dns:');
}
