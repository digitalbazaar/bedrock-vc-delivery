...
export async function getAuthorizationRequest({req, clientProfileId}) {
  ...
  while(true) {
    ...
    const clientProfile = step.openId.clientProfiles ?
      step.openId.clientProfiles[clientProfileId] : step.openId;
    if(!clientProfile) {
    ...
    if(!authorizationRequest) {
      ...
      if(!authorizationRequest) {
        ...
        // add / override params from step OID4VP client profile
        const {
...
export async function getOID4VPProtocols({workflow, exchange}) {
  ...
  // generate protocol URL for each client profile
  let protocols = {};
  for(const [clientProfileId, clientProfile] of clientProfiles) {
    ...
    const clientBaseUrl = clientProfileId !== undefined ?
      `${openIdRoute}/clients/${clientProfileId}` :
      `${openIdRoute}/client`;
    const searchParams = new URLSearchParams({
