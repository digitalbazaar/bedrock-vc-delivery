...
export async function createRoutes({
  ...
  const oid4vpBaseUrl = `${openIdRoute}/clients/:clientProfileId`;
  const routes = {
    // OID4VCI routes
    ...
    // OID4VP routes:
    // legacy routes map to "default" client profile:
    ...
    profiledAuthorizationRequest: `${oid4vpBaseUrl}/request`,
    profiledAuthorizationResponse: `${oid4vpBaseUrl}/response`
  };
...
async function _handleOid4vpAuthzRequest(req, res) {
  const {clientProfileId = 'default'} = req.params;
  let result;
  try {
    const {
      authorizationRequest,
      // FIXME: add values to indicate how to send response
    } = await oid4vp.getAuthorizationRequest({req, clientProfileId});
    // FIXME: use return value to send profiled response
