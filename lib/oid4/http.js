...
function _normalizeCredentials({verifiablePresentation}) {
  ...
  return verifiableCredential.map(vc => {
    ...
    // parse any mdoc into its non-VC format (includes base64 => base64url)
    if(vc.type === 'EnvelopedVerifiableCredential') {
      if(vc.id?.startsWith('data:application/mdoc;base64,')) {
        const buffer = Buffer.from(
          vc.id.slice('data:application/mdoc;base64,'.length), 'base64');
        return buffer.toString('base64url');
      } else if(vc.id?.startsWith('data:application/mdl;base64,')) {
        // deprecated `mdl` media type
        const buffer = Buffer.from(
          vc.id.slice('data:application/mdl;base64,'.length), 'base64');
        return buffer.toString('base64url');
      }
    }
    return vc;
