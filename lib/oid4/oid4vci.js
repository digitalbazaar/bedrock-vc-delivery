...
import {getWorkflowIssuerInstances} from '../helpers.js';
...
export async function getCredentialOffer({req}) {
  ...
  // FIXME: consider running offer creation code within `ExchangeProcessor`
  // to add offer generated details like `credential_identifiers` to the
  // results of the step; this would also cause the exchange to "start" when
  // the offer is retrieved for the first time instead of first credential
  // request time
...
export async function processAccessTokenRequest({req}) {
  ...
  // process `authorizationDetails`, if any
  if(authorizationDetails) {
    // FIXME: match `authorizationDetails` to requested supported
    // configurations and put associated `credential_identifiers` into
...
function _createCredentialConfigurationsSupported({workflow, exchange}) {
  const {
    openId: {
      expectedCredentialRequests, supportedCredentialConfigurations
    }
  } = exchange;

  // if explicitly provided, return it
  if(supportedCredentialConfigurations) {
...
async function _processExchange({
  ...
  const exchangeProcessor = new ExchangeProcessor({
    ...
    async prepareStep({exchange, step}) {
      ...
      // `proofs`
      // `credential_response_encryption`; not supported

      // FIXME: possibly map credential ID to:
      // `${localExchangeId}-${issueRequestIndex}`; this would cause IAE to
      // have to be used on a particular and singular step -- but that might
      // be fine for OID4VCI; if this is to be more opaque, each
      // `issueRequest` could have some ID generated for it and these could be
      // stored in the step's `openId` results, i.e.,:
      /*
      {
        issueRequestMetadata: [{
          issueRequestIndex: <index>,
          credential_identifier: <uuid>,
          // ?
          credential_configuration_id: ...,
      ...
      */
      ...
      if(isBatchRequest) {
        // draft 13 batch request
        credentialRequests = draft13.normalizeCredentialRequestsToVersion1({
          credentialRequests: req.body.credential_requests,
          supportedCredentialConfigurations
        });
      } else {
        // either 1.0+ or draft 13 request
      ...
      // FIXME: map chosen `credential_identifier` or matching configuration
      // to `issueRequest`
