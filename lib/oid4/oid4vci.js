...
import {
  getAlgFromJwk, getWorkflowIssuerInstances, setVariable
} from '../helpers.js';
  ...
  const exchangeProcessor = new ExchangeProcessor({
    ...
    async prepareStep({exchange, step}) {
      ...
      if(step.divpDidProofRequest || step.jwtDidProofRequest) {
        ...
        const credentialRequestProofResults = await Promise.all(
        ...
        const did = credentialRequestProofResults[0].did;
        if(credentialRequestProofResults.some(({did: d}) => did !== d)) {
          // FIXME: improve error
          throw new Error('every DID must be the same');
        }
        // store results in variables associated with current step
        exchange.variables.results[exchange.step] = {
          ...exchange.variables.results[exchange.step],
          // common use case of DID Authentication; provide `did` for ease
          // of use in templates
          did,
          credentialRequestProofResults
        };
      }
    },
...
async function _createOAuth2AccessToken({
  ...
  const alg = getAlgFromJwk({jwk: privateKeyJwk});
