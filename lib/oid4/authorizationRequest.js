...
export async function encode({
  workflow, clientProfile, authorizationRequest, requestMethod
} = {}) {
  // translate non-redirect_uri-scheme client IDs for backwards compatibility
  // with OID4VP draft 18; only draft 18 clients will use the `get` method and
  // 1.0+ clients will use `post` when `request_uri_method=post`
  const {
    client_id, client_id_scheme, request_uri_method
  } = authorizationRequest;
  if(requestMethod === 'get' && request_uri_method === 'post' &&
    client_id && client_id_scheme !== 'redirect_uri') {
