    ...
    try {
      ...
      while(true) {
        ...
        if(isInputRequired) {
          ...
          // 4.4.3. Save the exchange and return `response`.
        ...
        if(step.verifiablePresentation || issueToClient) {
          // 4.6.1. If `response` is not `null`: then save the exchange and
          // return the response.
          if(response) {
            // 4.6.1.1. If `response.verifiablePresentation` is not set,
        ...
        // 4.8. If `step.redirectUrl` is set:
        if(step.redirectUrl) {
          // 4.8.1. If `response` is `null` then set it to an empty object.
          if(!response) {
            response = {};
          }
          // 4.8.2. Set `response.redirectUrl` to `step.redirectUrl`.
          response.redirectUrl = step.redirectUrl;
        }

        // 4.9. If `step.nextStep` is not set then set `exchange.state` to
        // `complete`.
        if(!step.nextStep) {
          exchange.state = 'complete';
        } else {
          // 4.10. Otherwise, delete `exchange.variables.results[step.nextStep]`
        ...
        // 4.13. Set `receivedPresentation` to `null`.
