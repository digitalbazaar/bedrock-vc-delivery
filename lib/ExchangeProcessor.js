...
export class ExchangeProcessor {
  ...
  async process({receivedPresentation} = {}) {
    ...
    // 3. Initialize `step` and `response` to `null`.
    let step = null;
    let response = null;

    // 4. Process exchange step(s) until a response is generated, input from
    // the exchange client is required, or the exchange times out, optionally
    // saving any error thrown as `exchange.lastError`:
    try {
      ...
      while(true) {
        ...
        // 4.2. If `receivedPresentation` is set, then call the
        ...
        // 4.3. Set `isInputRequired` to the result of calling
        // `inputRequired({step, receivedPresentation})`.
        const isInputRequired = await inputRequired({
          workflow, exchange, step, receivedPresentation
        });

        // 4.4. If `isInputRequired` is true:
        if(isInputRequired) {
          // 4.4.1. If `response` is `null`, set it to an empty object.
          if(!response) {
            response = {};
          }

          // 4.4.2. If `step.verifiablePresentationRequest` is set, call
          ...
          // 4.4.3. Return `response`.
          return response;
        }

        // 4.5. Set `issueToClient` to `true` if `step.issueRequests` includes
        ...
        // 4.6. If `step.verifiablePresentation` is set or `issueToClient` is
        // `true`:
        if(step.verifiablePresentation || issueToClient) {
          // 4.6.1. If `response` is not `null` then return `response`:
          if(response) {
            return response;
          }

          // 4.6.2. Set `response` to an empty object.
          response = {};

          // 4.6.3. If `step.verifiablePresentation` is set, set
        ...
        // 4.7. Perform every issue request (optionally in parallel), returning
        ...
        // 4.7.1. For each issue request where `result` is set to an exchange
        // variable path or name, save the issued credential in the referenced
        // exchange variable.
        // 4.7.2. For each issue request where `result` is not specified, save
        ...
        // 4.8. If either `step.redirectUrl` or `step.nextStep` is set:
        if(step.redirectUrl || step.nextStep) {
          // 4.8.1. If `response` is `null`, set it to an empty object.
          if(!response) {
            response = {};
          }
          // 4.8.2. If `step.redirectUrl` is set then set
          // `response.redirectUrl` to `step.redirectUrl`.
          if(step.redirectUrl) {
            response.redirectUrl = step.redirectUrl;
          }
          // 4.8.3. If `step.nextStep` is set then set
          // `response.verifiablePresentationRequest` to an empty object
          // (to signal that the exchange has not terminated), delete
        ...
        // 4.9. If `step.nextStep` is not set, set `exchange.state` to
        // `complete`.
        if(!step.nextStep) {
          exchange.state = 'complete';
        }

        // 4.10. Save the exchange.
        await _updateExchange({workflow, exchange, meta, step});

        // 4.11. Set `receivedPresentation` to `null`.
        receivedPresentation = null;

        // 4.12. If `exchange.state` is `complete`, return `response` if it is
