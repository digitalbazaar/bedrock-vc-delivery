...
export class ExchangeProcessor {
   ...
   * @param {Function} [options.validateStep] - The `validateStep` handler.
   *
   * @returns {ExchangeProcessor} An `ExchangeProcessor` instance.
   */
  constructor({workflow, exchangeRecord, inputRequired, validateStep} = {}) {
  ...
  async process({receivedPresentation} = {}) {
    const {workflow, exchangeRecord, validateStep, inputRequired} = this;
    ...
    try {
      ...
      // the exchange client is required, or the exchange times out.
      ...
      while(true) {
        ...
        // 4.2. Call subalgorithm `validateStep`, passing `workflow`,
        // `exchange`, `step`, and `receivedPresentation` to perform any
        // protocol-specific custom step validation.
        await validateStep?.({workflow, exchange, step, receivedPresentation});
        ...
        if(receivedPresentation) {
          await _validateReceivedPresentation({
            workflow, exchange, step, receivedPresentation
          });
        }

        // 4.4. Set `isInputRequired` to the result of calling
        // `inputRequired({step, receivedPresentation})`.
        const isInputRequired = await inputRequired?.({
          workflow, exchange, step, receivedPresentation
        }) ?? false;

        // 4.5. If `isInputRequired` is true:
        if(isInputRequired) {
          // 4.5.1. If `response` is `null`, set it to an empty object.
          if(!response) {
            response = {};
          }

          // 4.5.2. If `step.verifiablePresentationRequest` is set, call
          ...
          // 4.5.3. Save the exchange and return `response`.
          await _updateExchange({workflow, exchange, meta, step});
          return response;
        }

        // 4.6. Set `issueToClient` to `true` if `step.issueRequests` includes
        ...
        // 4.7. If `step.verifiablePresentation` is set or `issueToClient` is
        // `true`:
        if(step.verifiablePresentation || issueToClient) {
          // 4.7.1. If `response` is not `null`
          if(response) {
            // 4.7.1.1. If `response.verifiablePresentation` is not set,
            ...
            // 4.7.1.2. Save the exchange.
            await _updateExchange({workflow, exchange, meta, step});
            // 4.7.1.3. Return `response`.
            return response;
          }

          // 4.7.2. Set `response` to an empty object.
          response = {};

          // 4.7.3. If `step.verifiablePresentation` is set, set
        ...
        // 4.8. Perform every issue request (optionally in parallel), returning
        ...
        // 4.8.1. For each issue request where `result` is set to an exchange
        // variable path or name, save the issued credential in the referenced
        // exchange variable.
        // 4.8.2. For each issue request where `result` is not specified, save
        ...
        // 4.9. If `step.redirectUrl` is set:
        if(step.redirectUrl) {
          // 4.9.1. If `response` is `null` then set it to an empty object.
          if(!response) {
            response = {};
          }
          // 4.9.2. Set `response.redirectUrl` to `step.redirectUrl`.
          response.redirectUrl = step.redirectUrl;
        }

        // 4.10. If `step.nextStep` is not set then set `exchange.state` to
        // `complete`.
        if(!step.nextStep) {
          exchange.state = 'complete';
        } else {
          // 4.11. Otherwise, delete `exchange.variables.results[step.nextStep]`
        ...
        // 4.12. Save the exchange.
        await _updateExchange({workflow, exchange, meta, step});

        // 4.13. If `exchange.state` is `complete`, return `response` if it is
        // not `null`, otherwise return an empty object.
        if(exchange.state === 'complete') {
          return response ?? {};
        }

        // 4.14. Set `receivedPresentation` to `null`.
...
async function _createVerifiablePresentationRequest({
  ...
  // property `verifiablePresentationRequest` set to
  // `response.verifiablePresentationRequest`.
  if(!exchange.variables.results) {
    exchange.variables.results = {};
  }
  exchange.variables.results[exchange.step] = {
    verifiablePresentationRequest: response.verifiablePresentationRequest
  };
}
...
async function _validateReceivedPresentation({
  workflow, exchange, step, receivedPresentation
}) {
  ...
  // `exchange.variables.results[exchange.step].verifiablePresentationRequest`
  // if it is set, otherwise set it to `undefined` (for subsequent steps,
  // an implementation may also set the challenge in an implementation-specific
  // way).
  const isInitialStep = _isInitialStep({workflow, exchange});
  const vpr = exchange.variables
    .results?.[exchange.step]?.verifiablePresentationRequest;
  const expectedChallenge = isInitialStep ? exchange.id : vpr?.challenge;
  ...
  const expectedDomain = vpr?.domain ??
  ...
  const verifyResult = await verify({
    ...
    verifyPresentationOptions,
    verifyPresentationResultSchema,
    verifiablePresentationRequest: vpr ?? step.verifiablePresentationRequest,
    presentation: receivedPresentation,
  ...
  // presentation returned from the verification process, throwing an error if
  // validation fails.
  if(presentationSchema && isEnvelopedPresentation) {
    validateVerifiablePresentation({
      schema: presentationSchema.jsonSchema,
