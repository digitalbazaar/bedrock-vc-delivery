    ...
    try {
      ...
      while(true) {
        ...
        if(receivedPresentationRequest) {
          await _validateReceivedPresentationRequest({
        ...
        if(step.verifiablePresentation || issueToClient) {
          ...
          if(response) {
            // 4.8.1.1. If `response.verifiablePresentation` is not set,
...
async function _validateReceivedPresentationRequest({
  exchange, step, receivedPresentationRequest
}) {
  ...
  // `exchange.variables.results[exchange.step]` so it can be used in
  // subsequent steps.
  if(!exchange.variables.results) {
    exchange.variables.results = {};
  }
  exchange.variables.results[exchange.step] = {
    ...exchange.variables.results[exchange.step],
    verifiablePresentationRequest: receivedPresentationRequest
  };
}
