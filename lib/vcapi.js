...
export async function getProtocols({req} = {}) {
  ...
  // FIXME: if `exchange.protocols` is set, use it, otherwise compute
  // the protocols...
  ...
  if(oid4vci.supportsOID4VCI({exchange})) {
    oid4vciProtocols = oid4vci.getOID4VCIProtocols({workflow, exchange});
  } else {
    // only add OID4VP protocols if OID4VCI is not supported to cover
    // OID4VCI+OID4VP combo case
    oid4vpProtocols = await oid4vp.getOID4VPProtocols({workflow, exchange});
  }
