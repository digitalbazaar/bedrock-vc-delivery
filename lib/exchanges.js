...
export async function insert({exchangerId, exchange}) {
  ...
  assert.optionalObject(exchange.vars);
  // FIXME: optional current step in the exchange
  assert.optionalNumber(exchange.step);
...
export async function get({exchangerId, id, explain = false} = {}) {
  ...
  // FIXME: might want to pass `complete: {$ne: false}`? ... that way
  // queries will fail fast for already-completed exchanges that still
...
async function _invalidateExchange({record}) {
  try {
    ...
    logger.error(`Could not mark exchange "${id}" invalid.`, {error});
  }
