...
export async function update({exchangerId, exchange, explain = false} = {}) {
  ...
  const query = {
    ...
    // FIXME: change previous state to be either `pending` or `active` to
    // complete: `'exchange.state': {$in: ['pending', 'active']}
...
export async function complete({exchangerId, exchange, explain = false} = {}) {
  ...
  // FIXME: allow for `active` state as well
