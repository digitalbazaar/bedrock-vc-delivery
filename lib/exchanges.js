...
export async function insert({exchangerId, exchange}) {
  ...
  // FIXME: is there a use case for an exchange that does not expire?
  // FIXME: exchange should have variables for use in a template (can be
  // empty) as well as the current step in the process -- to be coordinated
  // with the actual template(s) and step definitions provided in the exchanger
...
export async function get({exchangerId, id, explain = false} = {}) {
  ...
  // FIXME: might want to pass `state: {$ne: 'invalid'}`? ... that way
  // queries will fail fast for already-invalidated exchanges that still
...
export async function complete({exchangerId, id, explain = false} = {}) {
  ...
  // FIXME: mark the exchange state (so maybe exchange should have a state
  // of `pending`, `complete`, `invalid`?)
  // FIXME: after handling auto-revocation / notification, the exchange state
  // should be updated to `invalid` so auto-revocation / notification does
  // not happen again
}
