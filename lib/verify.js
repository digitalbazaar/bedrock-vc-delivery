...
export async function verify({
  ...
  return {
    verified,
    challengeUses,
    // return original VM result and w/both common key formats
    verificationMethod,
    multikey: await _toMultikey({vm: verificationMethod}),
...
export async function verifyCredentialRequestProof({
  ...
  if(diVp) {
    ...
    verifyResults.di_vp.push({
      did: result.did,
      // return original VM result and w/both common key formats
      verificationMethod: result.verifyResult.verificationMethod,
      multikey: await _toMultikey({vm: result.verifyResult.verificationMethod}),
  ...
  if(jwt) {
    ...
    const multikey = await _toMultikey({
...
// tries to convert to JWK and returns undefined if it cannot
...
// tries to convert to multikey and returns undefined if it cannot
async function _toMultikey({vm, jwk, controller}) {
  ...
  let keyPair;
  if(jwk) {
    if(jwk.crv === 'Ed25519' || jwk.alg === 'EdDSA') {
      keyPair = await Ed25519Multikey.fromJwk({jwk, controller});
    }
    if(jwk.crv?.startsWith('P-') || jwk.alg?.startsWith('ES')) {
