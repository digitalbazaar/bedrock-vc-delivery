...
export async function createRoutes({
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      */
      const {verifiablePresentation} = await _processCredentialRequests(
...
function _getAlgFromPrivateKey({privateKeyJwk}) {
  ...
  if(privateKeyJwk.kty === 'EC' && privateKeyJwk.crv?.startsWith('P-')) {
...
async function _processCredentialRequests({req, res, isBatchRequest}) {
  ...
  const credentialRequests = [req.body];
  if(expectedCredentialRequests.length > 1 && !isBatchRequest) {
...
function _validateCredentialRequests({
  credentialRequests, expectedCredentialRequests
}) {
  ...
  if(credentialRequests.length !== expectedCredentialRequests.length ||
    !credentialRequests.every(cr => expectedCredentialRequests.some(
      ({type, format}) => cr.type === type && cr.format === format))) {
    // FIXME: improve error
