  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      */
      ...
      // FIXME: produce appropriate format
      ...
      res.json({
        ...
        credential: response.verifiablePresentation.verifiableCredential[0]
      });
    }));
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      */
      ...
      // send VCs
      const {response: {verifiablePresentation}} = result;
      const responses = verifiablePresentation.verifiableCredential.map(
...
function _matchCredentialRequest(any, cr) {
  // `cr` must match `any` directly or if `any` is an array, an element of it
  any = Array.isArray(any) ? any : [any];
  for(const expected of any) {
    if(expected.format !== cr.format) {
      continue;
    }
    ...
    if(c1.length === c2.length && t1.length === t2.length &&
