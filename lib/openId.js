  ...
  if(currentStep) {
    ...
    if(step.openId) {
      ...
      if(!results?.[exchange.step]?.openId?.presentationSubmission) {
        // FIXME: optimize await double step-template processing that currently
  ...
  while(true) {
    ...
    if(!authorizationRequest) {
      ...
      if(!authorizationRequest) {
        ...
        // FIXME: move exchange update out of conditionals so that
        // if exchange state is `pending` it can be changed to `active` even
        // if the authorization request was preset (not auto-generated)
        ...
        exchange.sequence++;
        // FIXME: set exchange state to `active` from `pending`
        try {
