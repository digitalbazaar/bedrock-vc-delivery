...
export async function createRoutes({
  ...
  app.post(
    ...
    getExchange,
    asyncHandler(async (req, res) => {
      const result = await _processAuthorizationResponse({req});
      res.json(result);
    }));
}
...
async function _processAuthorizationResponse({req}) {
  const {config: exchanger} = req.serviceObject;
  const exchangeRecord = await req.getExchange();
  let {exchange} = exchangeRecord;

  // get `vp_token` as VP and get `presentation_submission`
  const {
