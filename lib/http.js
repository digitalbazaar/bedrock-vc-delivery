  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        if(openId) {
          /* If any expected credential requests were given, then we ensure
          that an issuer instance is available that matches each of the
          expected credential definition requests. For simple configs, a single
          issuer instance is used with only an `issue` zcap and only the
          `ldp_vc` format is supported. Otherwise, we can check to ensure
          ...
          if(expectedCredentialRequests) {
            ...
            */
            let {issuerInstances} = config;
            if(!issuerInstances) {
              if(config.zcaps.issue) {
