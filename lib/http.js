...
import * as oidc4vci from './oidc4vci/index.js';
...
export async function addRoutes({app, service} = {}) {
  ...
  // used to fetch exhange record in parallel
  const getExchange = asyncHandler(async (req, res, next) => {
    const {localId: exchangerId, exchangeId: id} = req.params;
    const localExchangerId = decodeLocalId({localId: exchangerId});
    req.exchange = exchanges.get({localExchangerId, id});
    next();
  });
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      // 2. send VCs / go to next step

      // FIXME: implement
      res.status(400).json({error: 'not implemented'});
    }));

  // create OIDC4VCI service to be used with each individual exchange
  await oidc4vci.authorization.createService(
