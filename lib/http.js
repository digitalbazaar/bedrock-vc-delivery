  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      if(oid4.supportsOID4VCI({exchange})) {
        ...
        // FIXME: add several different clients:
        // 1. unencrypted request+response
        // 2. unencrypted request+encrypted response
        // 3. encrypted/signed request+encrypted response
        // FIXME: /clients/<profile>/authorization/<response|request> ?
        // FIXME: look up profile in exchange for rules
        // FIXME: /client/authorization/<response|request> uses default profile
