...
export async function issue({exchanger, exchange} = {}) {
  ...
  if(credentialTemplates) {
    ...
    const credentials = await Promise.all(credentialTemplates.map(
      async ({template: t}) => {
        const {vc, ...rest} = variables;
        const parsedVc = vc ? JSON.parse(
          JSON.stringify(await jsonata(vc).evaluate(rest))) : null;
        return jsonata(t).evaluate(parsedVc ? {vc: parsedVc} : variables);
      }));
