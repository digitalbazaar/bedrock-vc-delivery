...
const EXPECTED_REDIRECT_URL = 'https://test.example/interactions/foo?iuv=1';

describe('exchange w/ VC-API delivery + "issueRequest.result"', () => {
  let capabilityAgent;
    ...
    if(stepCount === 1) {
      steps = {
        ...
        didAuthn: {
          ...
          // issue same VC twice, storing in both cases
          issueRequests: [{
            credentialTemplateId: 'urn:credential-template-1',
            result: '/results/didAuthn/store/vc1'
          }, {
            credentialTemplateId: 'urn:credential-template-1',
            result: '/results/didAuthn/store/vc2',
          ...
          redirectUrl: EXPECTED_REDIRECT_URL
        }
      };
    } else {
      steps = {
        ...
        issue: {
          ...
          issueRequests: [{
            credentialTemplateId: 'urn:credential-template-1',
            result: '/results/issue/store/vc1'
          }, {
            credentialTemplateId: 'urn:credential-template-1',
            result: '/results/issue/store/vc2',
            // use different variables
            variables: {
              credentialId: 'urn:different',
              issuanceDate: '2024-01-01T00:00:00Z',
  ...
  it('should pass when sending VP in single call w/one step', async () => {
    ...
    // post VP to get `redirectUrl` response
    const response = await httpClient.post(
      exchangeId, {agent, json: {verifiablePresentation}});
    should.exist(response?.data?.redirectUrl);
    response.data.redirectUrl.should.equal(EXPECTED_REDIRECT_URL);
    ...
    // exchange should be complete and contain the VP and original VC
    {
      ...
      try {
        ...
        // ensure both issued VCs were stored properly...

        // ensure DID in VC matches `did`
        should.exist(exchange.variables.results.didAuthn.store);
        exchange.variables.results.didAuthn.store
          .should.include.keys(['vc1', 'vc2']);
        const {vc1, vc2} = exchange.variables.results.didAuthn.store;
        should.exist(vc1?.credentialSubject?.id);
        should.exist(vc2?.credentialSubject?.id);
  ...
  it('should pass when sending VP in single call w/two steps', async () => {
    ...
    // exchange should be complete and contain the VP and original VC
    {
      ...
      try {
        ...
        // ensure DID in VC matches `did`
        should.exist(exchange.variables.results.issue.store);
        exchange.variables.results.issue.store
          .should.include.keys(['vc1', 'vc2']);
        const {vc1, vc2} = exchange.variables.results.issue.store;
        should.exist(vc1?.credentialSubject?.id);
        should.exist(vc2?.credentialSubject?.id);
        vc1.credentialSubject.id.should.equal(did);
        // ensure VC ID matches
