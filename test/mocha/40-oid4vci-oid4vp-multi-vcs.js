...
describe('exchange w/OID4VCI + "issueRequests"', () => {
  let capabilityAgent;
  ...
  it('should pass w/ pre-authorized code flow', async () => {
    ...
    const {
      ...
      variables: {
        ...
        // use convoluted path to test replacment variables can be found
        nested: [{}, {
          secondVC: {
    ...
    result.credentials.should.be.an('array');

    // check first VC
    {
      result.credentials[0].should.be.a('string');
      const {credential} = await unenvelopeCredential({
        envelopedCredential: result.credentials[0],
        format: credentialFormat
      });
    ...
    // check second VC
    {
      result.credentials[1].should.be.a('string');
      const {credential} = await unenvelopeCredential({
        envelopedCredential: result.credentials[1],
        format: credentialFormat
      });
      // ensure credential subject ID matches DID from secondVC vars
      should.exist(credential?.credentialSubject?.id);
      credential.credentialSubject.id.should.equal('did:example:1');
      // ensure VC ID matches
      should.exist(credential.id);
      credential.id.should.equal('urn:different');
    }
